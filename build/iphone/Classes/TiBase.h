...
// in simulator we redefine to format for Nnu Developer console
...
{\
	NSString * exceptionName = [@"org.nnu." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.nnu";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UINnuNativeItemNone = -1, 
	UINnuNativeItemSpinner = -2,
	UINnuNativeItemProgressBar = -3,
	
	UINnuNativeItemSlider = -4,
	UINnuNativeItemSwitch = -5,
	UINnuNativeItemMultiButton = -6,
	UINnuNativeItemSegmented = -7,
	
	UINnuNativeItemTextView = -8,
	UINnuNativeItemTextField = -9,
	UINnuNativeItemSearchBar = -10,
	
	UINnuNativeItemPicker = -11,
	UINnuNativeItemDatePicker = -12,
	
	UINnuNativeItemInfoLight = -13,
	UINnuNativeItemInfoDark = -14,
	
	UINnuNativeItemDisclosure = -15,
	
	UINnuNativeItemContactAdd = -16
};
